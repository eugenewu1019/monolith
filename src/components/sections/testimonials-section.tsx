...
export default function TestimonialsSection() {
  ...
  const testimonialCount = TESTIMONIALS_DATA.length;
  const testimonial = TESTIMONIALS_DATA[activeIndex];

  const goToTestimonial = useCallback(
    (index: number) => {
      setActiveIndex((index + testimonialCount) % testimonialCount);
    },
    [testimonialCount],
  );
    ...
    <section className="py-32 px-12 md:px-24 border-t border-white/5 bg-zodiac-black relative overflow-hidden">
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-12 items-start">
            ...
            >
              ...
              <div className="flex gap-1">
                {[...Array(testimonial.rating)].map((_, i) => (
                  <Star
                    key={i}
                    className="w-4 h-4 fill-zodiac-gold text-zodiac-gold"
                  />
                ))}
              </div>
          ...
          {/* Navigation Dots */}
          <div className="md:col-span-4 flex flex-col gap-6 md:min-h-[260px] md:items-end">
            <div className="flex gap-3 md:flex-col md:items-end">
              {TESTIMONIALS_DATA.map((_, i) => (
                <button
                  key={i}
                  type="button"
                  aria-label={`${locale === "zh-TW" ? "切換至評論" : "Show testimonial"} ${i + 1}`}
                  onClick={() => goToTestimonial(i)}
                  className={`transition-all duration-300 focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-zodiac-gold ${
                    i === activeIndex
                      ? "h-3 w-12 rounded-full bg-zodiac-gold"
            ...
            <div className="mt-2 flex items-center gap-2 md:mt-auto">
              <button
                type="button"
                aria-label={
                  locale === "zh-TW" ? "上一則評論" : "Previous testimonial"
                }
                onClick={showPrevious}
                className="flex h-11 w-11 items-center justify-center border border-white/15 bg-white/[0.03] text-white/55 transition-colors hover:border-zodiac-gold/50 hover:text-zodiac-gold focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-zodiac-gold"
              >
              ...
              <button
                type="button"
                aria-label={
                  locale === "zh-TW" ? "下一則評論" : "Next testimonial"
                }
                onClick={showNext}
                className="flex h-11 w-11 items-center justify-center border border-white/15 bg-white/[0.03] text-white/55 transition-colors hover:border-zodiac-gold/50 hover:text-zodiac-gold focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-zodiac-gold"
              >

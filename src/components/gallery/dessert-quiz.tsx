...
const CHEF_RECOMMENDATIONS: { [key: string]: { name: string; zhName: string; note: string; zhNote: string } } = {
    "calm-rich-stoic": {
        name: "Obsidian Tart",
        zhName: "黑曜石塔",
...
function getRecommendation(answers: string[]): { name: string; zhName: string; note: string; zhNote: string } {
...
function QuizModal({ isOpen, onClose, children }: { isOpen: boolean; onClose: () => void; children: React.ReactNode }) {
    ...
    // This is a standard hydration safety pattern for portals
...
export default function DessertQuiz() {
    ...
    const [result, setResult] = useState<{ name: string; zhName: string; note: string; zhNote: string } | null>(null);
    ...
    return (
        <>
            ...
            <div
                ...
                className="group relative h-[60vh] w-[40vh] min-w-[300px] flex-shrink-0 cursor-pointer overflow-hidden border border-dashed border-white/20 hover:border-zodiac-gold/40 transition-all duration-500 bg-transparent"
            >
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center p-8 text-center">
                    <p className="text-xs uppercase tracking-widest text-white/30 mb-4">{t("quiz.secretMenu")}</p>
                    <p className="text-lg font-serif text-white/60 mb-2">{t("quiz.stillHesitating")}</p>
                    <p className="text-sm text-white/40 font-zh-serif mb-6">{t("quiz.letChefChoose")}</p>
                ...
                >
                    ...
                    {!result ? (
                        <>
                            ...
                            <div className="flex gap-2 justify-center mb-8">
                                {QUESTIONS.map((_, i) => (
                                    <div
                                        key={i}
                                        className={`w-8 h-px transition-colors ${i <= currentQuestion ? "bg-zodiac-gold" : "bg-white/10"
                                            }`}
                                    />
                                ))}
                            </div>
                                ...
                                <div className="space-y-3">
                                    {QUESTIONS[currentQuestion].options.map((opt) => (
                                        ...
                                        >
                                            ...
                                            <span className="text-white/70 font-zh-serif">{t(opt.textKey)}</span>
                        ...
                        >
                            ...
                            <h2 className="text-3xl font-serif text-white mb-2">{locale === "zh-TW" ? result.zhName : result.name}</h2>
                            <p className="text-lg font-zh-serif text-white/50 mb-8">{locale === "zh-TW" ? result.name : result.zhName}</p>
                            ...
                            <div className="bg-white/5 border-l-2 border-zodiac-gold/50 p-6 text-left mb-8">
                                <p className="text-sm text-white/60 italic font-zh-serif leading-relaxed">
                                    {t("quiz.resultNote", { note: locale === "zh-TW" ? result.zhNote : result.note })}
                                </p>
                                <p className="text-xs text-white/30 mt-4">{t("quiz.chefRenzo")}</p>
                            </div>
